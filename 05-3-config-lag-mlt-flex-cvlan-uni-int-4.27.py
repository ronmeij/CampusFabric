##########
# Name: 		05-3-config-lag-mlt-flex-cvlan-uni-int.py
# Vers: 		4.27
# Date: 		260925
# Auth: 		Ronald Meijer
# Pyth: 		version 3
...
#				4.14	corrections for fa and fa-mgmt and lacp support on interfaces only
#				4.15	correction for fa and fa-mgmt an mlt should not have flex-uni enabled
#				4.16	corrections of error messages
#				4.17	corrections for flex-uni on MLT also
#				4.18	detects csv file delimiter
#				4.20	added support for .xlsx filte type
#				4.21	corrected smlt error on left side; column was wronly mapped 21 instead of 22
#				4.22	user can now select a particular sheet from the workbook
#				4.23	corrections c-vlan and flex-uni on LACP MLT
#				4.24	corrections c-vlan were not required
#				4.25	missing line in vlan_id_pr formatting
#				4.26	improved logging
#				4.27	str was missing in second part of script "vlan____id = [str(x).encode("ascii", "ignore") for x in vlan____id]"
#
##########
# Variables:
...
with open (filename) as f:
	...
	for shtidx, sheet_name in enumerate(sheet_names):
		print(f"    {shtidx+1}: {sheet_name}")
	
	while True:
		try:
			selected_sheet = int(input("Please select a sheet from the list of files or press '0' for all sheets in the workbook: [0] ") or "0")
		except ValueError:
			print("\n\t **** INFORM Please enter a valid integer for the index (a number please...)\n")
			continue
		else:
			...
			if selected_sheet == 0:
				print ("\n\t**** All sheets in the open workbook are selected\n")
				break
			if selected_sheet <= 0:
				print ("\n\t**** INFORM Cannot open this file, please enter a valid integer for the index\n")
				continue
			if selected_sheet >= len(sheet_names)+1:
				print ("\n\t**** INFORM Cannot open this file, please enter a valid integer for the index\n")
				continue
			if 1 <= selected_sheet <= len(sheet_names)+1:
				selected_sheet = sheet_names[selected_sheet-1]
				sheet_names = [selected_sheet]
				print("\n\t**** Selected sheet is " + selected_sheet + "\n")
			break
	...
	for sheet_name in sheet_names:
		...
		#for row in reader:
		# Read table for SWITCH#1 starting with row 6 down and till column 25 (12)

		for row in sheet.iter_rows(min_row=header_row_index+1, min_col=0, max_col=25, values_only=True):
			if row[0] is not None:
				valid_rows.append(row)

		for row in valid_rows:
			try:
				fa_mgmt_tg.append(row[7] if row[7] is not None else "")
				tagged_int.append(row[6] if row[6] is not None else "")
				lag_mlt_id.append(row[9] if row[9] is not None else "")
				intrfc__id.append(row[0] if row[0] is not None else "")
				intrfc__nm.append(row[3] if row[3] is not None else "")
				lacp___ena.append(row[8] if row[8] is not None else "")
				lag_mlt_pr.append(row[22] if row[22] is not None else "")
				def_vln_id.append(row[10] if row[10] is not None else "")
				vlan____id.append(row[11] if row[11] is not None else "")
				vlan_id_pr.append(row[24] if row[24] is not None else "")
			except IndexError:
				print("\n\n\t**** ERROR excel header not as expected!!!")
				quit()
		print("\n\n**** List data for left-hand switch: " + (switch_id1))
		print("\t**** fa/fa-mgmt (H)")
		...
		print(tagged_int)
		print("\n\t**** mlag/smlt-id (J)")
		lag_mlt_id = [str(x).encode("ascii", "ignore") for x in lag_mlt_id]
		lag_mlt_id = [x.decode() for x in lag_mlt_id]
		lag_mlt_id = [x.strip(" ") for x in lag_mlt_id]
		print(lag_mlt_id)
		print("\n\t**** mlag/smlt-id from peer switch (W)")
		lag_mlt_pr = [str(x).encode("ascii", "ignore") for x in lag_mlt_pr]
		...
		print(lenvlan_tmpid)
		for (num,vlans_int) in enumerate(vlan_tmpid):
			if vlans_int != '':
				print("\n\n\t\t**** vlan ids for the interface in excel line " + str(num + 2))
				print(vlans_int)
				vlan__list = vlans_int.split(',')
				vlan_nlist = []
				print("\t\t**** vlan ids as list for the interface in excel line " + str(num + 2))
				print(vlan__list)
				for vlan_value in vlan__list:
					if "-" in vlan_value:
						...
						for vlan____ids in vlan_range:
							...
							a2 = int(vlan_range[1])
							if a1 > a2:
								print("\n\n\t\t**** ERROR in vlan range " + str(vlan__list) + " on interface in excel line " + str(num + 2))
								quit()
						for vlanid in range(a1,(a2+1)):
							vlan_nlist.append(str(vlanid))
					else:
						vlan_nlist.append(vlan_value)
				print("\t\t**** vlan ids as complete list for the interface in excel line " + str(num + 2))
				vlan_nlist.sort()
		...
		print(vlan_orgid)
		print("\n\t**** vlan id peer")
		vlan_id_pr = [str(x).encode("ascii", "ignore") for x in vlan_id_pr]
		...
		vlan_id_pr = [x.replace(" ","") for x in vlan_id_pr]
		print(vlan_id_pr)
		answer = input("\nPress enter to continue")
		...
		print("\n\n**** Number of entries in the list (should be all the same):")
		print(lenfa_mgmt_tg)
		print(lentagged_int)
		print(lenlag_mlt_id)
		print(lenintrfc__id)
				...
				if "NNI" in lag_mlt_id[value]:
					...
					if lag_mlt_id[value]:
						...
						if uni___name != "SimvIST":
							print ("# ** CustUni, FlexUni and MLT ** Here we disable flex-uni at interface level")
						...
						if uni___name != "FlexUni":
							if "yes" in tagged_int[value]:
								if "0" == def_vln_id[value]:
									print ("# ** CustUni or SimplifiedIST and MLT ** Exclusively tagged interface")
									script = "untagged-frames-discard " 
									print(script)
									with open(outfil, 'a') as outfile:
										outfile.write("untagged-frames-discard\n")
							if "no" in tagged_int[value]:
								print ("# ** CustUni or SimplifiedIST and MLT ** Untagged interface")
						...
						if uni___name != "FlexUni":
							if "no" in tagged_int[value]:
								...
								with open(outfil, 'a') as outfile:
									outfile.write("no encapsulation dot1q\n")
						if "lacp-s" in lacp___ena[value]:
							print ("# ** LACP-S Interface ** ")
							script = "lacp key " + (intrfc__id[value]).replace('/', '') + " aggregation enable timeout-time short "
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("lacp key " + (intrfc__id[value]).replace('/', '') + " aggregation enable timeout-time short " + " \n")
							script = "lacp enable " + " "
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("lacp enable " + " \n")
							...
							with open(outfil, 'a') as outfile:
								outfile.write("# y\n")
						if "lacp-l" in lacp___ena[value]:
							print ("# ** LACP-L Interface ** ")
							script = "lacp key " + (intrfc__id[value]).replace('/', '') + " aggregation enable timeout-time long "
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("lacp key " + (intrfc__id[value]).replace('/', '') + " aggregation enable timeout-time long " + " \n")
							script = "lacp enable " + " "
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("lacp enable " + " \n")
							...
							with open(outfil, 'a') as outfile:
								outfile.write("# y\n")
						if uni___name != "SimvIST":
							print ("# ** FlexUni/CustUni ** The value of FA/FA-MGMT (column H) is: " + (fa_mgmt_tg[value]))
							if fa_mgmt_tg[value] == "fa":
								print ("# ** FlexUni/CustUni ** The value of FA/FA-MGMT (column H) must be 'fa'")
								script = "fa " + " "
								print(script)
								with open(outfil, 'a') as outfile:
									outfile.write("fa" + "\n")
							...
							if fa_mgmt_tg[value] == "fa-mgmt":
								print ("# ** FlexUni/CustUni ** The value of FA/FA-MGMT (column H) must be 'fa-mgmt'")
								script = "fa " + " "
								print(script)
								with open(outfil, 'a') as outfile:
									outfile.write("fa" + "\n")
								...
								with open(outfil, 'a') as outfile:
									outfile.write("fa enable" + "\n")
								if uni___name == "FlexUni":
									print ("# ** FlexUni ** The value of FA/FA-MGMT (column H) must be 'fa-mgmt'")
									if "yes" in tagged_int[value]:
										print ("# ** FlexUni ** The value of FA/FA-MGMT (column H) must be 'fa-mgmt' and TAGGING (Column G) is set to 'yes' ")
										if vlan____id[value]:
											...
											if "," in vlan____id[value]:
												...
												cvid_value = int(vlan____id[value])
												cvid_intgr = int(vlan____id[value])
												try:
													script = "fa management i-sid " + str(vlan__elan[cvid_intgr]) + " c-vid " + str(cvid_value) + " "
													print(script)
													with open(outfil, 'a') as outfile:
														outfile.write("fa management i-sid " + str(vlan__elan[cvid_intgr]) + " c-vid " + str(cvid_value) + "\n")
												except KeyError:
													print("\n\n#### ERROR VLAN to I-SID mapping not found !!!" + " ")
													print("\n\t# check if the VLANid '" + str(cvid_value) + "' exists in dictionary file!!!" + " \n\n")
													quit()
									if "no" in tagged_int[value]:
										print ("# ** FlexUni ** The value of FA/FA-MGMT (column H) must be 'fa-mgmt' and TAGGING (Column G) is set to 'no' ")
										if vlan____id[value]:
											if "," in  vlan____id[value]:
												cvid__list = vlan____id[value].split(',')
												for cvid_value in cvid__list:
												...
												try:
													...
													quit()
								if uni___name == "CustUni":
									print ("# ** CustUni ** The value of FA/FA-MGMT (column H) must be 'fa-mgmt'")
									if "yes" in tagged_int[value]:
										print ("# ** CustUni ** The value of FA/FA-MGMT (column H) must be 'fa-mgmt' and TAGGING (Column G) is set to 'yes' ")
										if vlan____id[value]:
											if "," in vlan____id[value]:
												cvid__list = vlan____id[value].split(',')
												for cvid_value in cvid__list:
													cvid_intgr = int(cvid_value)
													script = "fa management i-sid " + str(fa_mgmt_is) + " c-vid " + (cvid_value) + " "
													print(script)
													with open(outfil, 'a') as outfile:
														outfile.write("fa management i-sid " + str(fa_mgmt_is) + " c-vid " + (cvid_value) + "\n")
														break
											else:
												cvid_value = int(vlan____id[value])
												script = "fa management i-sid " + str(fa_mgmt_is) + " c-vid " + str(cvid_value) + " "
												print(script)
												with open(outfil, 'a') as outfile:
													outfile.write("fa management i-sid " + str(fa_mgmt_is) + " c-vid " + str(cvid_value) + "\n")	
									if "no" in tagged_int[value]:
										print ("# ** CustUni ** The value of FA/FA-MGMT (column H) must be 'fa-mgmt' and TAGGING (Column G) is set to 'no' ")
										if vlan____id[value]:
											if "," in vlan____id[value]:
												cvid__list = vlan____id[value].split(',')
												for cvid_value in cvid__list:
				...
				if "NNI" in lag_mlt_id[value]:
					...
					if lag_mlt_id[value]:
						if lag_mlt_id[value] not in dup_lag_mlt_id_lft:
							...
							print ("# ** List of MLT-ids that have been used so far : " + str(dup_lag_mlt_id_lft))
			...
			if uni___name == "FlexUni":
				...
				for value in range(0,lenlag_mlt_id):
					if "NNI" in lag_mlt_id[value]:
						...
						if not "fa-mgmt" in fa_mgmt_tg[value]:
		...
		with open(outfil, 'a') as outfile:
			...
			for row in valid_rows:
				try:
					fa_mgmt_tg.append(row[20] if row[20] is not None else "")
					tagged_int.append(row[19] if row[19] is not None else "")
					lag_mlt_id.append(row[22] if row[22] is not None else "")
					intrfc__id.append(row[13] if row[13] is not None else "")
					intrfc__nm.append(row[16] if row[16] is not None else "")
					lacp___ena.append(row[21] if row[21] is not None else "")
					lag_mlt_pr.append(row[9] if row[9] is not None else "")
					def_vln_id.append(row[23] if row[23] is not None else "")
					vlan____id.append(row[24] if row[24] is not None else "")
			...
			print("\n\n**** List data for right-hand switch: " + (switch_id2))
			print("\t**** fa/fa-mgmt (U)")
			fa_mgmt_tg = [x.encode("ascii", "ignore") for x in fa_mgmt_tg]
			fa_mgmt_tg = [x.decode() for x in fa_mgmt_tg]
			fa_mgmt_tg = [x.strip(" ") for x in fa_mgmt_tg]
			...
			lag_mlt_id = [x.strip(" ") for x in lag_mlt_id]
			print(lag_mlt_id)
			print("\n\t**** mlag/smlt-id from peer switch (J)")
			lag_mlt_pr = [str(x).encode("ascii", "ignore") for x in lag_mlt_pr]
			lag_mlt_pr = [x.decode() for x in lag_mlt_pr]
			lag_mlt_pr = [x.strip(" ") for x in lag_mlt_pr]
			print(lag_mlt_pr)
						...
						if lag_mlt_id[value]:
							...
							if "lacp-l" in lacp___ena[value]:
								...
								with open(outfil, 'a') as outfile:
									outfile.write("# y\n")
							if uni___name != "SimvIST":
								print ("# ** FlexUni/CustUni ** The value of FA/FA-MGMT (column U) is: " + (fa_mgmt_tg[value]))
								if fa_mgmt_tg[value] == "fa":
									print ("# ** FlexUni/CustUni ** The value of FA/FA-MGMT (column U) must be 'fa'")
									script = "fa " + " "
									print(script)
									with open(outfil, 'a') as outfile:
										outfile.write("fa" + "\n")
									...
									with open(outfil, 'a') as outfile:
										outfile.write("fa enable" + "\n")
								if fa_mgmt_tg[value] == "fa-mgmt":
									print ("# ** FlexUni/CustUni ** The value of FA/FA-MGMT (column U) must be 'fa-mgmt'")
									script = "fa " + " "
									print(script)
									with open(outfil, 'a') as outfile:
										outfile.write("fa" + "\n")
									...
									with open(outfil, 'a') as outfile:
										outfile.write("fa enable" + "\n")
									if uni___name == "FlexUni":
										print ("# ** FlexUni ** The value of FA/FA-MGMT (column U) must be 'fa-mgmt'")
										if "yes" in tagged_int[value]:
											print ("# ** FlexUni ** The value of FA/FA-MGMT (column U) must be 'fa-mgmt' and TAGGING (Column T) is set to 'yes' ")
											if vlan____id[value]:
												...
												if "," in vlan____id[value]:
													...
													try:
														...
														quit()
										if "no" in tagged_int[value]:
											print ("# ** FlexUni ** The value of FA/FA-MGMT (column H) must be 'fa-mgmt' and TAGGING (Column T) is set to 'no' ")
											if vlan____id[value]:
												if "," in  vlan____id[value]:
													cvid__list = vlan____id[value].split(',')
													for cvid_value in cvid__list:
													...
													try:
														...
														quit()
									if uni___name == "CustUni":
										print ("# ** CustUni ** The value of FA/FA-MGMT (column U) must be 'fa-mgmt'")
										if "yes" in tagged_int[value]:
											print ("# ** CustUni ** The value of FA/FA-MGMT (column U) must be 'fa-mgmt' and TAGGING (Column T) is set to 'yes' ")
											if vlan____id[value]:
												if "," in vlan____id[value]:
													cvid__list = vlan____id[value].split(',')
													for cvid_value in cvid__list:
													...
													with open(outfil, 'a') as outfile:
														outfile.write("fa management i-sid " + str(fa_mgmt_is) + " c-vid " + str(cvid_value) + "\n")	
										if "no" in tagged_int[value]:
											print ("# ** CustUni ** The value of FA/FA-MGMT (column U) must be 'fa-mgmt' and TAGGING (Column T) is set to 'no' ")
											if vlan____id[value]:
												if "," in vlan____id[value]:
													cvid__list = vlan____id[value].split(',')
													for cvid_value in cvid__list:
					...
					if "NNI" in lag_mlt_id[value]:
						...
						if lag_mlt_id[value]:
							if lag_mlt_id[value] not in dup_lag_mlt_id_rght:
								...
								print ("# ** List of MLT-ids that have been used so far : " + str(dup_lag_mlt_id_rght))
								script = "\n\nmlt " + lag_mlt_id[value] + ' enable name MLT--' + lag_mlt_id[value] + ' '
								print(script)
								with open(outfil, 'a') as outfile:
									outfile.write("\n\nmlt " + lag_mlt_id[value] + ' enable name MLT--' + lag_mlt_id[value] + '\n')
												...
												if vlan____id[value]:
													if "," in vlan____id[value]:
														...
														try:
															...
															quit()
											if "no" in tagged_int[value]:
												print (" ** FlexUni ** The value of FA/FA-MGMT (column U) must be 'fa-mgmt' and TAGGING (Column T) is set to 'no' ")
												if vlan____id[value]:
													if "," in vlan____id[value]:
														cvid__list = vlan____id[value].split(',')
														for cvid_value in cvid__list:
