	...
	if not "NONE" in switch_idx:
		...
		for value in range(0,lenvlan____id):
			if "nocreate" in ip_intadrx[value]:
				script = '# Not create vlan ' + vlan____id[value] + ' on this switch (nocreate) '
				print(script)
				with open(outfil, 'a') as outfile:
					outfile.write('# Not create vlan ' + vlan____id[value] + ' on this switch (nocreate) \n\n') 
			else:
				...
				if ist___name == "NormIST":
					if not isid____id[value]:
						script = '# Error no i-sid defined for VLAN ' + vlan____id[value] + ' '
						print(script)
						with open(outfil, 'a') as outfile:
							outfile.write('# Error no i-sid defined for VLAN ' + vlan____id[value] + ' \n\n') 
					else:
		...
		for value in range(0,lenvlan____id):
			if "nocreate" in ip_intadrx[value] or "mc-config-lite" in ip_intadrx[value]:
				if "nocreate" in ip_intadrx[value]:
					script = '# The vlan ' + vlan____id[value] + ' is not created on this switch, no IP interface'
					print(script)
					with open(outfil, 'a') as outfile:
						outfile.write('# The vlan ' + vlan____id[value] + ' is not created on this switch, no IP interface \n\n') 
				if "mc-config-lite" in ip_intadrx[value]:
					...
					script = "#ip igmp routed-spb-querier-addr A.B.C.D" + " "
					print(script)
					with open(outfil, 'a') as outfile:
						outfile.write("#ip igmp routed-spb-querier-addr A.B.C.D" + " \n")
					script = "exit" + " "
					print(script)
					with open(outfil, 'a') as outfile:
						outfile.write("exit" + " \n\n")
				...
				if dvr___gtwy[value]:
					if ist___name == "SimpIST":
						...
						if dvr___gtwy[value] == ip_intadrx[value]:
							script = "ip address " + ip_intadrx[value] + ip_adr_msk[value] + " dvr-one-ip "
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("ip address " + ip_intadrx[value] + ip_adr_msk[value] + " dvr-one-ip \n")
						else:
							script = "ip address " + ip_intadrx[value] + ip_adr_msk[value] + " "
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("ip address " + ip_intadrx[value] + ip_adr_msk[value] + " \n")
					...
					if ":" in ip_intadrx[value]:
						...
						script = "ipv6 interface address " + ip_intadrx[value] + ip_adr_msk[value] + " "
						print(script)
						with open(outfil, 'a') as outfile:
							outfile.write("ipv6 interface address " + ip_intadrx[value] + ip_adr_msk[value] + " \n")
					else:
						script = "ip address " + ip_intadrx[value] + ip_adr_msk[value] + " "
						print(script)
						with open(outfil, 'a') as outfile:
				...
				if vrrp1_gtwy[value]:
					if ":" in vrrp1_gtwy[value]:
						...
						script = "ipv6 vrrp address " + vrrp1___id[value] + " global " +  vrrp1_gtwy[value] + "/" + ip_adr_msk[value] + " "
						print(script)
						with open(outfil, 'a') as outfile:
							outfile.write("ipv6 vrrp address " + vrrp1___id[value] + " global " +  vrrp1_gtwy[value] + "/" + ip_adr_msk[value] + "\n")
				...
				if vrrp2_gtwy[value]:
					if ":" in vrrp2_gtwy[value]:
						...
						script = "ipv6 vrrp address " + vrrp2___id[value] + " global " +  vrrp2_gtwy[value] + "/" + ip_adr_msk[value] + " "
						print(script)
						with open(outfil, 'a') as outfile:
							outfile.write("ipv6 vrrp address " + vrrp2___id[value] + " global " +  vrrp2_gtwy[value] + "/" + ip_adr_msk[value] + "\n")
			...
			if "nocreate" in ip_intadrx[value] or "mc-config-lite" in ip_intadrx[value]:
				script = '# The vlan ' + vlan____id[value] + ' is not created on this switch, no DHCP-relay'
				print(script)
				with open(outfil, 'a') as outfile:
					outfile.write('# The vlan ' + vlan____id[value] + ' is not created on this switch, no DHCP-relay \n\n') 
			else:
				if dhcp_rel_1[value]:
					if ip_intadrx[value]:
						if int(vrf_____id[value]) >= 1:
