...
#				2.10	disable ssh x509v3-auth enable
#				2.11	disable ssh authentication hmac-sha1 and encryption blowfish-cbc
#				2.12	added 'dhcp' option for oob
##########
...
for value in range(0,lenpromptname):
	...
	script = "no ssh x509v3-auth enable " + " "
	print (script)
	with open(out_file, 'a') as outfile:
		outfile.write("no ssh x509v3-auth enable " + " \n")
	script = "no ssh authentication-type hmac-sha1 " + " "
	print (script)
	with open(out_file, 'a') as outfile:
		outfile.write("no ssh authentication-type hmac-sha1 " + " \n")
	script = "no ssh encryption-type blowfish-cbc " + " "
	print (script)
	with open(out_file, 'a') as outfile:
		outfile.write("no ssh encryption-type blowfish-cbc " + " \n")
	...
	if not "dhcp" in ob_mgmt_ip[value]:
	...
	if "dhcp" in ob_mgmt_ip[value]:
		script = "mgmt dhcp-client oob" + " "
		print (script)
		with open(out_file, 'a') as outfile:
			outfile.write("mgmt dhcp-client oob" + " \n\n")
