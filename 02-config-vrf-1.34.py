...
with open (filename) as f:
	...
	print("\n\t**** VRFid:")
	vrf_____id = [str(x).strip(" ") for x in vrf_____id]
	print(vrf_____id)
	print("\n\t**** VRF name:")
	vrf___name = [x.strip(" ") for x in vrf___name]
	print(vrf___name)
	print("\n\t**** L3-VSN:")
	vrf___isid = [str(x).strip(" ") for x in vrf___isid]
	print(vrf___isid)
	print("\n\t**** IPVPN:")
	vrf__ipvpn = [str(x).strip(" ") for x in vrf__ipvpn]
...
for value in range(0,lenvrf_____id):
	if int(vrf_____id[value]) >= 1:
		...
		with open(outfil, 'a') as outfile:
			outfile.write("exit " + " \n\n")
		script = 'i-sid name ' + vrf___isid[value] + ' "' + vrf___name[value] + '" \n\n'
		print(script)
		with open(outfil, 'a') as outfile:
			outfile.write('i-sid name ' + vrf___isid[value] + ' "' + vrf___name[value] + '" \n\n')
