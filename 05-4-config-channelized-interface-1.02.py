...
#				1.01	corrections
#				1.02	select slpp-guard or slpp-packet-rx
##########
...
#	slpp_name	guard or pkt_rx
#	slpp_type	slpp-guard or slpp packet-rx
#	slpp_th_s1	slpp packet-rx threshold selector1
#	slpp_th_s2	slpp packet-rx threshold selector2
#	slpp_th_1	slpp packet-rx threshold peer a
#	slpp_th_2	slpp packet-rx threshold peer b
##########
# Run:      python3 05-4-config-channelized-interface-x.xx.py
##########
...
while True:
	try:
		slpp_type = int(input("\nSelect the SLPP-type:\n 1 for SLPP-guard \n 2 for SLPP packet-rx  \n  : [1] ") or "1")
	except ValueError:
		...
		if slpp_type >= 3:
			print("\nPlease select a value between 1 and 2")
			continue
		# slpp-type number was succesfully parsed
		break
if slpp_type == 1:
	slpp_name = "guard"
elif slpp_type == 2:
	slpp_name = "pkt_rx"
print("\n")
# slpp-name was successfully set
if slpp_type == 2:
	while True:
		try:
			slpp_th_s = int(input("\nSelect the 'slpp packet-rx-threshold' for peer switch A [1-500] : [50] ") or "50")
		except ValueError:
			print("Should be a value from 1 to 500")
			continue
		else:
			if slpp_th_s <= 0:
				print("\nPlease select a value between 1 and 500")
				continue
			if slpp_th_s >= 501:
				print("\nPlease select a value between 1 and 500")
				continue
			# slpp-th_1 was succesfully parsed
			slpp_th_1 = slpp_th_s
			print("\n")
			break
	while True:
		try:
			slpp_th_s = int(input("\nSelect the 'slpp packet-rx-threshold' for peer switch B [1-500] : [150] ") or "150")
		except ValueError:
			print("Should be a value from 1 to 500")
			continue
		else:
			...
			if slpp_th_s >= 501:
				print("\nPlease select a value between 1 and 500")
				continue
			# slpp-th_2 was succesfully parsed
			slpp_th_2 = slpp_th_s
			print("\n")
			break
...
if "-" in interface:
	...
	for int in range(int_b,(int_c+1)):
		...
		if (slpp_name == "guard"):
			...
			script = "# Peer switch A:" + " "
			print(script)
			with open(out_file, 'a') as outfile:
				outfile.write("\n# Peer switch A" + " \n")
			
			script = "slpp packet-rx-threshold " + str(slpp_th_1) + " "
			print(script)
			with open(out_file, 'a') as outfile:
				outfile.write("slpp packet-rx-threshold " + str(slpp_th_1) + " \n")
			script = "slpp packet-rx " + " "
			print(script)
			with open(out_file, 'a') as outfile:
				outfile.write("slpp packet-rx " + " \n\n")

			script = "# Peer switch B:" + " "
			print(script)
			with open(out_file, 'a') as outfile:
				outfile.write("# Peer switch B" + " \n")
			
			script = "slpp packet-rx-threshold " + str(slpp_th_2) + " "
			print(script)
			with open(out_file, 'a') as outfile:
				outfile.write("slpp packet-rx-threshold " + str(slpp_th_2) + " \n")
			script = "slpp packet-rx " + " "
			print(script)
			with open(out_file, 'a') as outfile:
				outfile.write("slpp packet-rx " + " \n\n")
