...
#				1.63	restored checks with str(x)
#				1.64	the string 'local' as i-sid will remove the line from the dictionary
# 
##########
# Variables:
#	vlan____id	vlan-id
...
print("\t**** VLANid:")
vlan____id = [str(x).strip(" ") for x in vlan____id]
print(vlan____id)
print("\t**** I-SID:")
isid____id = [str(x).strip(" ") for x in isid____id]
isid____id = [x.lower() for x in isid____id]
print(isid____id)
...
for isid in result:
	...
	for key  in listOfKeys:
		print("\t\t\t VLAN: " + str(key))
		if 'local' in str(isid):
			print("\t **** INFORM local VLAN: " + str(key) + " is removed from dictionary\n")
			del vlan__isid_sorted[key]
