...
# Auth: 		Ronald Meijer
# Pyth: 		version 3
# Dependency:	01-spbm-param-detail_multi-area-ACME_v1.0.xlsx or 01-spbm-param-detail_multi-area-ACME_v1.1.xlsx
# Dependency:	openpyxl
...
# 				3.10	added support for xlsx file type
#				3.11	correct is__mep_id as string value
#				3.12	correct ist_vlanid and ist___isid as string value
#				3.13	if no mgmt clip is configured we will set the mgmt vlan as topology-ip
#				3.14	bvlans can now be changed
#				3.15	support for L3 MGMT VLAN interface
##########
...
with open (filename) as f:
	...
	if ist___type == 2:
		while True:
			try:
				bvlan14051 = int(input("\nSelect VLANid for BVLAN-1:\n" + 
				" BVLAN-1 : [4051] ") or "4051")
			except ValueError:
				print("Should be a value from 2 to 4058")
				continue
			else:
				if bvlan14051 <= 1:
					print("\nPlease select a value between 2 and 4058")
					continue
				if bvlan14051 >= 4059:
					print("\nPlease select a value between 2 and 4058")
					continue
				#bvlan-1 number was succesfully parsed
				break
		#bvlan14051 was successfully set

		while True:
			try:
				bvlan14052 = int(input("\nSelect VLANid for BVLAN-2:\n" + 
				" BVLAN-1 : [4052] ") or "4052")
			except ValueError:
				print("Should be a value from 3 to 4059")
				continue
			else:
				if bvlan14052 <= 2:
					print("\nPlease select a value between 3 and 4059")
					continue
				if bvlan14052 >= 4060:
					print("\nPlease select a value between 3 and 4059")
					continue
				if bvlan14052 == bvlan14051:
					print("\n BVLAN-2 cannot be the same as BVLAN-1 \nPlease select a value between 3 and 4059")
					continue
				if bvlan14052 < bvlan14051:
					print("\n BVLAN-2 should not have a lower value as BVLAN-1, please review values \nPlease select a value between 3 and 4059")
					continue
				#bvlan-2 number was succesfully parsed
				break
		#bvlan14052 was successfully set
	...
	print('\n\n\t\tNumber of valid rows in excel sheet =', len(valid_rows))

	for row in valid_rows:
		try:
			bmac__peer.append(row[31] if row[31] is not None else "")
			bmac__virt.append(row[30] if row[30] is not None else "")
			channelize.append(row[5] if row[5] is not None else "")
			dns_domnme.append(row[49] if row[49] is not None else "")
			dns_serv_1.append(row[50] if row[50] is not None else "")
			dns_serv_2.append(row[51] if row[51] is not None else "")
			dns_serv_3.append(row[52] if row[52] is not None else "")
			dvr___func.append(row[23] if row[23] is not None else "")
			dvr_clstid.append(row[34] if row[34] is not None else "")
			dvr_domain.append(row[24] if row[24] is not None else "")
			hm_areanme.append(row[53] if row[53] is not None else "")
			is__mep_id.append(row[27] if row[27] is not None else "")
			is__sys_id.append(row[28] if row[28] is not None else "")
			is_area_id.append(row[25] if row[25] is not None else "")
			is_loopbck.append(row[17] if row[17] is not None else "")
			is_sysname.append(row[14] if row[14] is not None else "")
			isis__auth.append(row[45] if row[45] is not None else "")
			ist___isid.append(row[33] if row[33] is not None else "")
			ist_clustr.append(row[29] if row[29] is not None else "")
			ist_ip_add.append(row[35] if row[35] is not None else "")
			ist_peerip.append(row[36] if row[36] is not None else "")
			ist_subnet.append(row[37] if row[37] is not None else "")
			ist_vlanid.append(row[32] if row[32] is not None else "")
			mgmt__clip.append(row[21] if row[21] is not None else "")
			mgmt__isid.append(row[20] if row[20] is not None else "")
			mgmt__vlan.append(row[19] if row[19] is not None else "")
			mgmtvlanip.append(row[18] if row[18] is not None else "")
			mlt1____id.append(row[11] if row[11] is not None else "")
			mlt1_membr.append(row[10] if row[10] is not None else "")
			mlt2____id.append(row[13] if row[13] is not None else "")
			mlt2_membr.append(row[12] if row[12] is not None else "")
			mlt_metric.append(row[42] if row[42] is not None else "")
			mstp__prio.append(row[43] if row[43] is not None else "")
			nick__name.append(row[26] if row[26] is not None else "")
			nni__ports.append(row[9] if row[9] is not None else "")
			nni_metric.append(row[41] if row[41] is not None else "")
			ntp_serv_1.append(row[38] if row[38] is not None else "")
			ntp_serv_2.append(row[39] if row[39] is not None else "")
			portmemrem.append(row[6] if row[6] is not None else "")
			promptname.append(row[0] if row[0] is not None else "")
			rm_areanme.append(row[54] if row[54] is not None else "")
			rm_area_id.append(row[55] if row[55] is not None else "")
			rm_nicknme.append(row[56] if row[56] is not None else "")
			rm_issysid.append(row[57] if row[57] is not None else "")
			rm_ho_nnip.append(row[58] if row[58] is not None else "")
			rm_ex_nnip.append(row[59] if row[59] is not None else "")
			rm_ho__mlt.append(row[60] if row[60] is not None else "")
			slpp_value.append(row[8] if row[8] is not None else "")
			snmpsyscon.append(row[16] if row[16] is not None else "")
			snmpsysloc.append(row[15] if row[15] is not None else "")
			sysloghost.append(row[40] if row[40] is not None else "")
			uni__ports.append(row[7] if row[7] is not None else "")
			trnsfl__ip.append(row[46] if row[46] is not None else "")
			trnsfl_pwd.append(row[48] if row[48] is not None else "")
			trnsfl_usr.append(row[47] if row[47] is not None else "")
			vlacp_isis.append(row[44] if row[44] is not None else "")
		except IndexError:
	...
	print(ist_peerip)
	print("\n\t**** vIST VLAN id:")
	ist_vlanid = [str(x).encode("ascii", "ignore") for x in ist_vlanid]
	ist_vlanid = [x.decode() for x in ist_vlanid]
	ist_vlanid = [x.strip(" ") for x in ist_vlanid]
	print(ist_vlanid)
	print("\n\t**** vIST VLAN I-SID:")
	ist___isid = [str(x).encode("ascii", "ignore") for x in ist___isid]
	ist___isid = [x.decode() for x in ist___isid]
	...
	print(is_area_id)
	print("\n\t**** ISIS MEP-id:")
	is__mep_id = [str(x).encode("ascii", "ignore") for x in is__mep_id]
	is__mep_id = [x.decode() for x in is__mep_id]
	...
	mgmtvlanip = [x.lower() for x in mgmtvlanip]
	print(mgmtvlanip)
	print("\n\t**** Management VLAN id:")
...
for value in range(0,lenpromptname):
	...
	if trnsfl__ip[value]:
		...
		if trnsfl_usr[value]:
			...
			if trnsfl_pwd[value] == "":
				print("\n\n\t**** DATA error: transferfile ftp user without password is not supported -> Excel file line " + str(value + 4) + " ")
				quit()
			else:				
...
# UniBern requires udp-port 529
	...
	if "yes" in ist_clustr[value]:
		if bmac__virt[value]:
			...
			print("\n\n\t**** DATA error: for switch vIST (cluster) you must specify a virtual bmac -> Excel file line " + str(value + 4) + " ")
			quit()
	...
	if portmemrem[value]:
		...
		print("\n\n\t**** DATA error: all switch ports (including channelized) must be removed from vlan 1 -> Excel file line " + str(value + 4) + " ")
		quit()
	...
	if mgmtvlanip[value]:
		if mgmt__vlan[value] == "":
			print("\n\n\t**** DATA error: with mgmt vlan ip you must specify a mgmt vlan id -> Excel file line " + str(value + 4) + " ")
			quit()
		else:		
			if mgmt__isid[value] == "":
				print("\n\n\t**** DATA error: with mgmt vlan ip you must specify an i-sid for mgmt vlan -> Excel file line " + str(value + 4) + " ")
				quit()
			else:
				if "l3/" in mgmtvlanip[value]:
					script = '# Note that the mgmt vlan ' + str(mgmt__vlan[value]) + ' will be created as L3 mgmt vlan '
					print(script)
					with open(out_file, 'a') as outfile:
						outfile.write('# Note that the mgmt vlan ' + str(mgmt__vlan[value]) + ' will be created as L3 mgmt vlan ' + ' \n')
				if "leaf" in dvr___func[value]:
					if "l3/" in mgmtvlanip[value]:
						print("\n\n\t**** DATA error: L3 mgmt vlan is not supported on DVR leaf -> Excel file line " + str(value + 4) + " ")
						quit()
					else:
					...
					if "l3/" in mgmtvlanip[value]:
						script = '# Note that the mgmt vlan ' + str(mgmt__vlan[value]) + ' will be configured as L3 mgmt vlan '
						print(script)
						with open(out_file, 'a') as outfile:
							outfile.write('# Note that the mgmt vlan ' + str(mgmt__vlan[value]) + ' will be configured as L3 mgmt vlan ' + ' ')
						script = "interface vlan " + str(mgmt__vlan[value]) + " "
						print(script)
						with open(out_file, 'a') as outfile:
							outfile.write("\ninterface vlan " + str(mgmt__vlan[value]) + " \n")
						script = "ip address " + (mgmtvlanip[value]).strip('l3/') + " " 
						print(script)
						with open(out_file, 'a') as outfile:
							outfile.write("ip address " + (mgmtvlanip[value]).strip('l3/') + "\n")
						script = "mgmt enable " + " " 
						print(script)
						with open(out_file, 'a') as outfile:
							outfile.write("mgmt enable " + "\n")
						script = "exit " + " " 
						print(script)
						with open(out_file, 'a') as outfile:
							outfile.write("exit " + "\n\n")
	...
	if ist___name == "NormIST":
		...
		if nick__name[value] == "":
			print("\n\n\t**** DATA error: for spbm configurations you must specify a unique nick-name -> Excel file line " + str(value + 4) + " ")
			quit()
		else:		
		...
		script = "spbm 1 b-vid " + str(bvlan14051) + "," + str(bvlan14052) + " primary " + str(bvlan14051) + " "
		print(script)
		with open(out_file, 'a') as outfile:
			outfile.write("spbm 1 b-vid " + str(bvlan14051) + "," + str(bvlan14052) + " primary " + str(bvlan14051) + " \n")
		if not "leaf" in dvr___func[value]:
		...
		if "yes" in ist_clustr[value]:
			...
			if bmac__peer[value] == "":
				print("\n\n\t**** DATA error: for switch vIST (cluster) you must specify the peer system-id -> Excel file line " + str(value + 4) + " ")
				quit()
			else:		
		...
		if not "leaf" in dvr___func[value]:
			if "yes" in ist_clustr[value]:
				if ist_subnet[value]:
					...
					print("\n\n\t**** DATA error: for switch vIST (cluster) you must specify the ist subnet -> Excel file line " + str(value + 4) + " ")
					quit()					
				if ist_vlanid[value]:
					...
					print("\n\n\t**** DATA error: for switch vIST (cluster) you must specify the ist vlan id -> Excel file line " + str(value + 4) + " ")
					quit()					
				if ist___isid[value]:
					...
					print("\n\n\t**** DATA error: for switch vIST (cluster) you must specify the ist vlan i-sid -> Excel file line " + str(value + 4) + " ")
					quit()
				...
				if ist_ip_add[value]:
					...
					print("\n\n\t**** DATA error: for switch vIST (cluster) you must specify the ist vlan ip address -> Excel file line " + str(value + 4) + " ")
					quit()
				script = "exit " + " "
				print(script)
				with open(out_file, 'a') as outfile:
...
# setup for simplified vist - ist vlan
	if ist___name == "SimpIST":
		if "yes" in ist_clustr[value]:
			if ist_subnet[value]:
				...
				print("\n\n\t**** DATA error: for switch IST (cluster) you must specify the ist subnet -> Excel file line " + str(value + 4) + " ")
				quit()	
			if ist_vlanid[value]:
				...
				print("\n\n\t**** DATA error: for switch IST (cluster) you must specify the ist vlan id -> Excel file line " + str(value + 4) + " ")
				quit()
			script = "interface vlan " + ist_vlanid[value] + " "
			print(script)
			with open(out_file, 'a') as outfile:
			...
			if ist_ip_add[value]:
				...
				print("\n\n\t**** DATA error: for switch IST (cluster) you must specify the ist vlan ip address -> Excel file line " + str(value + 4) + " ")
				quit()
			script = "exit " + " "
			print(script)
			with open(out_file, 'a') as outfile:
	...
	if ist___name == "NormIST":
		script = "vlan create " + str(bvlan14051) + ' name "B-VLAN-1" type spbm-bvlan '
		print(script)
		with open(out_file, 'a') as outfile:
			outfile.write("vlan create " + str(bvlan14051) + ' name "B-VLAN-1" type spbm-bvlan \n')
		script = "vlan create " + str(bvlan14052) + ' name "B-VLAN-2" type spbm-bvlan '
		print(script)
		with open(out_file, 'a') as outfile:
			outfile.write("vlan create " + str(bvlan14052) + ' name "B-VLAN-2" type spbm-bvlan \n\n')
	...
	if not "leaf" in dvr___func[value]:
		if "yes" in ist_clustr[value]:
			if ist_peerip[value]:
				...
				print("\n\n\t**** DATA error: for switch (v)IST (cluster) you must specify the peer ist ip address -> Excel file line " + str(value + 4) + " ")
				quit()
	...
	if uni__ports[value]:
		...
		print("\n\n\t**** DATA warning: no uni ports are defined on this switch -> Excel file line " + str(value + 4) + " ")
		answer()

	if "ctrl" in dvr___func[value]:
		if dvr_domain[value] == "":
			print("\n\n\t**** DATA error: if the switch is a dvr controller you must specify a dvr domain id -> Excel file line " + str(value + 4) + " ")
			quit()
		else:
	...
	if "leaf" in dvr___func[value]:
		if dvr_domain[value] == "":
			print("\n\n\t**** DATA error: if the switch is a dvr leaf you must specify the dvr domain id -> Excel file line " + str(value + 4) + " ")
			quit()
		else:
		...
		if "yes" in ist_clustr[value]:
			if ist_peerip[value]:
				...
				print("\n\n\t**** DATA error: for a dvr leaf vIST (cluster) you must specify the peer ist ip address -> Excel file line " + str(value + 4) + " ")
				quit()
	...
	if nni__ports[value]:
		...
		print("\n\n\t**** DATA warning: no nni ports are defined on this switch -> Excel file line " + str(value + 4) + " ")
		answer()
	...
	if ist___name == "NormIST":
		if not "leaf" in dvr___func[value]:
			if is_loopbck[value]:
				...
				print("\n\n\t**** DATA warning: for a spbm switch it is recommended to have an isis ip-source (loopback) address -> Excel file line " + str(value + 4) + " ")
				answer()
