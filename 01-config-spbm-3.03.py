...
with open (filename) as f:
	...
	while True:
		try:
			...
			" 3 for Europe London\n 4 for Europe Madrid\n" +
			" 5 for Europe Paris\n 6 for Europe Rome\n" +
			" 7 for Europe Zurich\n 8 for Africa Nairobi\n  : [7] ") or "7")
		except ValueError:
			print("Should be a value from 1 to 8")
			continue
		else:
			if time__zone <= 0:
				print("\nPlease select a value between 1 and 8")
				continue
			if time__zone >= 9:
				print("\nPlease select a value between 1 and 8")
				continue
	...
	print(uni__ports)
	print("\n\t**** SLPP setup on UNI interfaces (slpp-guard or packet-rx-threhold):")
...
for value in range(0,lenpromptname):
	...
	if sysloghost[value]:
		script = "# web-server help-tftp " + sysloghost[value] + ":/vsp8k_81000_help "
		print(script)
		with open(out_file, 'a') as outfile:
			outfile.write("# web-server help-tftp " + sysloghost[value] + ":/vsp8k_81000_help \n\n")
...
# setup for simplified vist - ist vlan
	...
	if uni__ports[value]:
		...
		if "slpp-guard" in slpp_value[value]:
