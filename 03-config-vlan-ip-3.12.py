...
# Auth: 		Ronald Meijer
# Pyth: 		version 3
# Dependency:	03-vlan-id-name-isid-ip_ACME_v1.0.xlsx
# Dependency:	openpyxl
...
#				3.01	added anycast-gateway support
#				3.02	yes in any/dvr column will also select dvr (to be compatible with earleir versions)
#				3.03	restored check in vlan-id str(x)
#				3.04	vrrp priority is now splitted and applied correctly, no post editing required anymore
#				3.05	added the i-sid name to the configuration
#				3.06	added option "local" for i-sid value
#				3.07	corrected syntax anycast-gateway
#				3.08	rmslt interfaces cannot be created in state disabled
#				3.09	accept netmask in dotted format also 255.255.255.0
#				3.10	accept vrrp v2 as suffix in vrrp-id1
#				3.11	added default settings in vrrp for advertisement-timer time and holddown-timer
#				3.12	anycast gateway will remove slpp from vid (as beb will generate slpp packets anyway)
#
##########
# Variables:
#	dhcp_rel_1 
...
#	ip_adr_msk	if the address masks start with 'd' the ip interface is created in the disabled state
...
#	vrrp1_gtwy 
#	vrrp2_gtwy 
#	vrrp1___id	vrrp instance id
#	vrrp2___id 
#	vrrp1_prio	string like "200 and 190 and 180 and 160" for 4 interfaces
#	vrrp2_prio 
...
#
#	filename	list of the *.xlsx files in working directory
...
with open (filename) as f:
	...
	for row in valid_rows:
		try:
			...
			anydvr_ena.append(row[6] if row[6] is not None else "")
			anydvr_gtw.append(row[7] if row[7] is not None else "")
			...
			if row[11] is not None:
				# Split the value by "and" and strip whitespace
				sub_array = [int(num.strip()) for num in row[11].split(" and ")]
				vrrp1_prio.append(sub_array)
			else:
				# Set row[11] to an empty string if it's None
				vrrp1_prio.append("")
			# vrrp2_prio.append(row[15] if row[15] is not None else "")			
			if row[15] is not None:
				# Split the value by "and" and strip whitespace
				sub_array = [int(num.strip()) for num in row[11].split(" and ")]
				vrrp2_prio.append(sub_array)
			else:
				# Set row[15] to an empty string if it's None
				vrrp2_prio.append("")
		except IndexError:
	...
	print(dhcp_rel_4)
	print("\t**** anycast-gateway or dvr enable (any for anycast-gateway, dvr for dvr):")
	anydvr_ena = [x.encode("ascii", "ignore") for x in anydvr_ena]
	anydvr_ena = [x.decode() for x in anydvr_ena]
	anydvr_ena = [x.strip(" ") for x in anydvr_ena]
	anydvr_ena = [x.lower() for x in anydvr_ena]
	print(anydvr_ena)
	print("\t**** anycast-gateway or dvr gateway IP address:")
	anydvr_gtw = [x.encode("ascii", "ignore") for x in anydvr_gtw]
	anydvr_gtw = [x.decode() for x in anydvr_gtw]
	anydvr_gtw = [x.strip(" ") for x in anydvr_gtw]
	print(anydvr_gtw)
	...
	print(ip_intadrB)
	print("\t**** ip netmask (a 'd' infront of the netmask will create the IP interface in state disabled):")
	...
	print(rsmlt_edge)
	print("\t**** vlan id:")
	vlan____id = [str(x).encode("ascii", "ignore") for x in vlan____id]
	...
	print(vlan__name)
	print("\t**** l2vsn i-sid:")
	isid____id = [str(x).encode("ascii", "ignore") for x in isid____id]
	isid____id = [x.decode() for x in isid____id]
	isid____id = [x.strip(" ") for x in isid____id]
	print(isid____id)
	print("\t**** vrf id (0=grt):")
	vrf_____id = [str(x).encode("ascii", "ignore") for x in vrf_____id]
	...
	# vrrp1_prio = [str(x).encode("ascii", "ignore") for x in vrrp1_prio]
	# vrrp1_prio = [x.decode() for x in vrrp1_prio]
	# vrrp1_prio = [x.strip(" ") for x in vrrp1_prio]
	print(vrrp1_prio)
	print("\t**** ip vrrp priorities instance 2:")
	# vrrp2_prio = [str(x).encode("ascii", "ignore") for x in vrrp2_prio]
...
lenanydvr_ena = len(anydvr_ena)
lenanydvr_gtw = len(anydvr_gtw)
...
def switch_fnc(switch_idx, ip_intadrx, swInd):
			...
			if "nocreate" in ip_intadrx[value]:
				...
				if ist___name == "NormIST":
					if not isid____id[value]:
						...
						if "local" in isid____id[value]:
							script = '# INFORM This is a "local" VLAN ' + vlan____id[value] + ' ' + vlan__name[value] + ', no i-sid is required'
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write('# INFORM This is a local VLAN ' + vlan____id[value] + ' ' + vlan__name[value] + ', no i-sid is required \n\n') 
						else:
							script = "vlan i-sid " + str(vlan____id[value]) + " " + isid____id[value] + " "
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("vlan i-sid " + str(vlan____id[value]) + " " + isid____id[value] + " \n")
							script = 'i-sid name ' + isid____id[value] + ' "' + vlan__name[value] + '" '
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write('i-sid name ' + isid____id[value] + ' "' + vlan__name[value] + '" \n\n')
			...
			if "nocreate" in ip_intadrx[value] or "mc-config-lite" in ip_intadrx[value]:
				...
				if anydvr_gtw[value]:
					if ist___name == "SimpIST":
						script = '# Error Anycast-gateway or DVR-IP cannot be configured with Simplified vIST for VLAN ' + str(vlan____id[value]) + ' '
						print(script)
						with open(outfil, 'a') as outfile:
							outfile.write('# Error Anycast-gateway or DVR-IP cannot be configured with Simplified vIST for VLAN ' + str(vlan____id[value]) + ' \n\n')
					else:
						if anydvr_gtw[value] == ip_intadrx[value]:
							if "any" in anydvr_ena[value]:
								if "." in ip_adr_msk[value]:
									script = "ip anycast-gateway one-ip " + ip_intadrx[value] + " " + (ip_adr_msk[value]).strip('d/') + " " 
									print(script)
									with open(outfil, 'a') as outfile:
										outfile.write("ip anycast-gateway one-ip " + ip_intadrx[value] + " " + (ip_adr_msk[value]).strip('d/') + " \n")
								else:
									script = "ip anycast-gateway one-ip " + ip_intadrx[value] + (ip_adr_msk[value]).strip('d') + " " 
									print(script)
									with open(outfil, 'a') as outfile:
										outfile.write("ip anycast-gateway one-ip " + ip_intadrx[value] + (ip_adr_msk[value]).strip('d') + " \n")
							if "dvr" in anydvr_ena[value]:
								if "." in ip_adr_msk[value]:
							...
							if "any" in anydvr_ena[value]:
								if "." in ip_adr_msk[value]:
									...
									script = "ip anycast-gateway " + anydvr_gtw[value] + " " 
									print(script)
									with open(outfil, 'a') as outfile:
										outfile.write("ip anycast-gateway " + anydvr_gtw[value] + " \n")
								else:
									script = "ip address " + ip_intadrx[value] + (ip_adr_msk[value]).strip('d') + state + " "
									print(script)
									with open(outfil, 'a') as outfile:
							...
							if "dvr" in anydvr_ena[value]:
								if "." in ip_adr_msk[value]:
									...
									with open(outfil, 'a') as outfile:
										outfile.write("ip address " + ip_intadrx[value] + " " + (ip_adr_msk[value]).strip('d/') + state + " \n")
									script = "dvr gw-ipv4 " + anydvr_gtw[value] + " "
									print(script)
									with open(outfil, 'a') as outfile:
										outfile.write("dvr gw-ipv4 " + anydvr_gtw[value] + " \n")
								else:
									script = "ip address " + ip_intadrx[value] + (ip_adr_msk[value]).strip('d') + state + " "
									print(script)
									with open(outfil, 'a') as outfile:
							...
							if "yes" in anydvr_ena[value]:
								if "." in ip_adr_msk[value]:
									...
									with open(outfil, 'a') as outfile:
										outfile.write("dvr gw-ipv4 " + anydvr_gtw[value] + " \n")
					if not "no" in anydvr_ena[value]:
						if ist___name == "SimpIST":
							script = '# Error Anycast-gateway or DVR-IP cannot be enabled with Simplified vIST for VLAN ' + str(vlan____id[value]) + ' '
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write('# Error Anycast-gateway or DVR-IP cannot be enabled with Simplified vIST for VLAN ' + str(vlan____id[value]) + ' \n\n')
						else:
							if not anydvr_gtw[value]:
								script = '# Error Anycast-gateway or DVR-IP cannot be enabled without DVR gateway address for VLAN ' + str(vlan____id[value]) + ' '
								print(script)
								with open(outfil, 'a') as outfile:
									outfile.write('# Error Anycast-gateway or DVR-IP cannot be enabled without DVR gateway address for VLAN ' + str(vlan____id[value]) + ' \n\n')
							else:
								if "any" in anydvr_ena[value]:
									script = "ip anycast-gateway enable " + " " 
									print(script)
									with open(outfil, 'a') as outfile:
										outfile.write("ip anycast-gateway enable " + " \n")
								if "dvr" in anydvr_ena[value]:
									script = "dvr enable"
									print(script)
									with open(outfil, 'a') as outfile:
					...
					if ":" in ip_intadrx[value]:
						...
						if "yes" in rsmlt__ena[value]:
							if "." in ip_adr_msk[value]:
								script = "ip address " + ip_intadrx[value] + " " + (ip_adr_msk[value]).strip('d/') + " "
								print(script)
								with open(outfil, 'a') as outfile:
									outfile.write("ip address " + ip_intadrx[value] + " " + (ip_adr_msk[value]).strip('d/') + " \n")
							else:
								script = "ip address " + ip_intadrx[value] + (ip_adr_msk[value]).strip('d') + " "
								print(script)
								with open(outfil, 'a') as outfile:
				...
				if "yes" in igmp_proxy[value]:
					if not "yes" in anydvr_ena[value]:
				...
				if vrrp1_gtwy[value]:
					# print("\t# Setting VRRP Priority for :\n\t# vrrp1_gtwy=", vrrp1_gtwy[value], "\n\t# List of priorities=", vrrp1_prio, " \n\t# Selecting prio from above list at row=", value, "th and column=", swInd, "th, row and col starting at 0")
					if ":" in vrrp1_gtwy[value]:
						...
						script = "ipv6 vrrp " + vrrp1___id[value] + " priority " + str(vrrp1_prio[value][swInd % len(vrrp1_prio[value])]) + " "
						print(script)
						with open(outfil, 'a') as outfile:
							outfile.write("ipv6 vrrp " + vrrp1___id[value] + " priority " + str(vrrp1_prio[value][swInd % len(vrrp1_prio[value])]) + " \n")
					else:
						if "v2" in vrrp1___id[value]:
							script = "ip vrrp version 2"
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("ip vrrp version 2 \n")
							script = "ip vrrp address " + (vrrp1___id[value]).replace('v2','') + " " + vrrp1_gtwy[value] + " "
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("ip vrrp address " + (vrrp1___id[value]).replace('v2','') + " " + vrrp1_gtwy[value] + " \n")
							script = "ip vrrp " + (vrrp1___id[value]).replace('v2','') + " backup-master enable "
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("ip vrrp " + (vrrp1___id[value]).replace('v2','') + " backup-master enable \n")
							script = "ip vrrp " + (vrrp1___id[value]).replace('v2','') + " priority " + str(vrrp1_prio[value][swInd % len(vrrp1_prio[value])]) + " "
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("ip vrrp " + (vrrp1___id[value]).replace('v2','') + " priority " + str(vrrp1_prio[value][swInd % len(vrrp1_prio[value])]) + " \n")
							script = "ip vrrp " + (vrrp1___id[value]).replace('v2','') + " adver-int 1"
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("ip vrrp " + (vrrp1___id[value]).replace('v2','') + " adver-int 1 \n")
							script = "ip vrrp " + (vrrp1___id[value]).replace('v2','') + " holddown-timer 0"
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("ip vrrp " + (vrrp1___id[value]).replace('v2','') + " holddown-timer 0 \n")
						else:
							...
							script = "ip vrrp " + vrrp1___id[value] + " priority " + str(vrrp1_prio[value][swInd % len(vrrp1_prio[value])]) + " "
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("ip vrrp " + vrrp1___id[value] + " priority " + str(vrrp1_prio[value][swInd % len(vrrp1_prio[value])]) + " \n")
							script = "ip vrrp " + vrrp1___id[value] + " adver-int 1"
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("ip vrrp " + vrrp1___id[value] + " adver-int 1 \n")
							script = "ip vrrp " + vrrp1___id[value] + " holddown-timer 0"
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("ip vrrp " + vrrp1___id[value] + " holddown-timer 0 \n")
				if "yes" in vrrp1__ena[value]:
					if ":" in vrrp1_gtwy[value]:
						...
						if "v2" in vrrp1___id[value]:
							script = "ip vrrp " + (vrrp1___id[value]).replace('v2','') + " enable"
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("ip vrrp " + (vrrp1___id[value]).replace('v2','') + " enable \n")
						else:
				...
				if vrrp2_gtwy[value]:
					if ":" in vrrp2_gtwy[value]:
						...
						script = "ipv6 vrrp " + vrrp2___id[value] + " priority " + str(vrrp2_prio[value][swInd % len(vrrp2_prio[value])]) + " "
						print(script)
						with open(outfil, 'a') as outfile:
							outfile.write("ipv6 vrrp " + vrrp2___id[value] + " priority " + str(vrrp2_prio[value][swInd % len(vrrp2_prio[value])]) + " \n")
					else:
						if "v2" in vrrp2___id[value]:
							script = "ip vrrp address " + (vrrp2___id[value]).replace('v2','') + " " + vrrp2_gtwy[value] + " "
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("ip vrrp address " + (vrrp2___id[value]).replace('v2','') + " " + vrrp2_gtwy[value] + " \n")
							script = "ip vrrp " + (vrrp2___id[value]).replace('v2','') + " backup-master enable "
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("ip vrrp " + (vrrp2___id[value]).replace('v2','') + " backup-master enable \n")
							script = "ip vrrp " + (vrrp2___id[value]).replace('v2','') + " priority " + str(vrrp2_prio[value][swInd % len(vrrp2_prio[value])]) + " "
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("ip vrrp " + (vrrp2___id[value]).replace('v2','') + " priority " + str(vrrp2_prio[value][swInd % len(vrrp2_prio[value])]) + " \n")
							script = "ip vrrp " + (vrrp2___id[value]).replace('v2','') + " adver-int 1"
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("ip vrrp " + (vrrp2___id[value]).replace('v2','') + " adver-int 1 \n")
							script = "ip vrrp " + (vrrp2___id[value]).replace('v2','') + " holddown-timer 0"
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("ip vrrp " + (vrrp2___id[value]).replace('v2','') + " holddown-timer 0 \n")
						else:
							...
							script = "ip vrrp " + vrrp2___id[value] + " priority " + str(vrrp2_prio[value][swInd % len(vrrp2_prio[value])]) + " "
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("ip vrrp " + vrrp2___id[value] + " priority " + str(vrrp2_prio[value][swInd % len(vrrp2_prio[value])]) + " \n")
							script = "ip vrrp " + vrrp2___id[value] + " adver-int 1"
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("ip vrrp " + vrrp2___id[value] + " adver-int 1 \n")
							script = "ip vrrp " + vrrp2___id[value] + " holddown-timer 0"
							print(script)
							with open(outfil, 'a') as outfile:
								outfile.write("ip vrrp " + vrrp2___id[value] + " holddown-timer 0 \n")
				...
				if "any" in anydvr_ena[value]:
					script = '# Anycast Gateway is configured on VLAN ' + str(vlan____id[value]) + ' SLLP can be disabled'
					print(script)
					with open(outfil, 'a') as outfile:
						outfile.write('\n# Anycast Gateway is configured on VLAN ' + str(vlan____id[value]) + ' SLLP can be disabled')
					script = "no slpp vid " + str(vlan____id[value]) + " "
					print(script)
					with open(outfil, 'a') as outfile:
						outfile.write("\nno slpp vid " + str(vlan____id[value]) + " \n\n")
			elif "yes" in igmp_snoop[value]:
				script = '# No IP address is configured on VLAN ' + str(vlan____id[value]) + ' but IGMP snooping is required'
				print(script)
				with open(outfil, 'a') as outfile:
					outfile.write('\n# No IP address is configured on VLAN ' + str(vlan____id[value]) + ' but IGMP snooping is required')
				script = "interface vlan " + str(vlan____id[value]) + " "
				print(script)
				with open(outfil, 'a') as outfile:
					outfile.write("\ninterface vlan " + str(vlan____id[value]) + " \n")
